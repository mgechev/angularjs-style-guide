...
*コントローラーの依存関係に則って名前を付けましょう。これによって、より読みやすいコードを書けるようになります:
...
* scope変数の代わりに文字列のように使いたいときや、 `style` 属性を `{{ }}` と一緒に使ってみたいとき、`ng-style` ディレクティブなら scope の変数を object-like パラメーターのように使えます:
