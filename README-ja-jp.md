...
* ディレクティブに関連するファイル(例：templates, CSS/SASS files, JavaScript)は全て１つのディレクトリに格納してします。このスタイルを選択する場合は、プロジェクト全体に一貫してこのスタイルを適用します。
...
* メソッド呼び出し（子が親へアクセスしたいと思った時に利用可能）や `$emit` `$broadcast` `$on` メソッドで他のコントローラ内にあるものと連携を取るようにします。emitとbroadcastするメッセージ は最小限に保ちましょう。
...
* できるだけ軽量なフィルタを作りましょう。フィルターは `$digest` ループ内で頻繁に呼ばれるため、フィルターが遅いとあなたのアプリ全体が遅くなります。
...
* アンカータグの `href` の内容が動的な場合は、 `href` の代わりに `{{ }}` と `ng-href` を使いましょう。
